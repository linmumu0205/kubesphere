...
class WebAPP(object):
    ...
    def __init__(
        ...
        #self.address = 'https://ks-console-eslkzcgr.c.kubesphere.cloud:30443/login'
    ...
    def _get_steps(self) -> list:
        steps = [
            ...
            Step({
                ...
                'target': 'id=username',  # 使用 "username" 的定位
                'command': Command.TYPE
            }),
            Step({
                'step': 3,
                'value': self.account.secret,  # 密码
                'target': 'id=password',  # 使用 "password" 的定位
                'command': Command.TYPE
            }),
            Step({
                'step': 4,
                'value': None,
                'target': 'xpath=//*[@class="button button-control button-size-normal"]',  # 点击登录按钮的定位
                'command': Command.CLICK
            })
        ]
        return steps
